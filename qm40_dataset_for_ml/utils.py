...
def call_heavy_atom_count(
    dataset: str, smile_column: str = "Column1", threshold: int = 30
) -> pd.DataFrame:
    df = pd.read_csv(dataset)
...
def input_file_creator(current_dir_path: str) -> None:
    list_dir = [
        name
        for name in os.listdir(current_dir_path)
        if os.path.isdir(os.path.join(current_dir_path, name))
    ]
    for current_dir in list_dir:
        command = []
...
def sbatch_create_submission(current_dir_path: str) -> None:
    ...
    for current_dir in list_dir:
        ...
        command.append(
            "cp ${gaussian_input_file} ${gaussian_checkpoint} ${job_directory}/ \n"
        )
...
def lmod_clac(path: str, current_dir: str, target_folder: str) -> None:
    ...
    try:
        ...
        print(
            f"Lmod out file {lmod_out_name} not found in {subfolder_path}, skipping lmod_out data."
        )
...
# remove files with imaginary frequency
def imaginary_freq_checker(curr_path: str, current_file: str) -> bool:
    current_gout = f"{current_file}_run.out"
    curr_file = os.path.join(curr_path, current_gout)
    with open(curr_file, "r") as f:
        lines = f.readlines()
